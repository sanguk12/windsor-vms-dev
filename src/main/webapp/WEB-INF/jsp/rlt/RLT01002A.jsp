						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="RLT01002A.eventYM" />
												</td>
												<td class="td-input">
				                                 ...
				                                 <td class="td-cond">
													<fmt:message key="RLT01002A.activityCD" />
												</td>
											...
											<tr>
				                                ...
				                                <td class="td-cond">
													<fmt:message key="RLT01002A.subSegmentCD" />
												</td>
												<td class="td-input">
				                            ...
				                            <tr>
				                            	<td class="td-cond">
													<fmt:message key="RLT01002A.addrTpCD1" />
												</td>
												<td class="td-input">
                	...
                	<td height="1">
                		<table border="0" cellspacing="0" cellpadding="0" width="100%" >
			        		...
			        		<tbody>
			        			<tr>
									<td class="tit_text01">
										<fmt:message key="RLT01002A.myAppr" />
									</td>
									<td class="tit_text01">
										<span id="myAppr" />
	                                </td>
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0100201S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"apprStateCDName;apprStateCD;resultTpCD;guidPayRate;excPayTpCD;"+
	  			"activityCD;chgID;");
...
// 	  	if (grid != null) grid.clear();
	...
	function RequestApp() {
		if($("#myAppr").text() == "") {
			alert("<fmt:message key="RLT01002A.msg1"/>");
			return;
		}
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.apprStateCD, index[i]) != "10") {
					alert("<fmt:message key="RLT01002A.msg2"/>");
					return;
				}
				//선지원(20)이 아닌경우
				if(grid.getCellText(_col.activityCD, index[i]) != "20") {
					...
					if(payAmt_keyman != tpayAmt) {
						...
						alert("<fmt:message key="RLT01002A.msg3"/>");
						return;
					}
				}
				...
				if(payRate > guidPayRate && guidPayRate < 70) {
					if(grid.getCellText(_col.commt, index[i]) == "") {
						...
						alert("<fmt:message key="RLT01002A.msg7"/>");
						return;
					}
				}
			}
		...
		if(!confirm("<fmt:message key="RLT01002A.msg4"/>")) {
       		return;
       	}
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.apprStateCD, index[i]) != "20") {
					alert("<fmt:message key="RLT01002A.msg5"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="RLT01002A.msg6"/>")) {
       		return;
       	}
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "RLT0100203S");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("empID", "${params.empID}");
	  	table.setParameter("diffNo", $("#diffNo").val());
	  	table.setParameter("activityCD", $("#activityCD_S").val());
		...
		var headerText = [
			"<fmt:message key="RLT01002A.programCD"/>"
	   		, "<fmt:message key="RLT01002A.activityCD"/>"
	   		, "<fmt:message key="RLT01002A.venueCD"/>"
	   		, "<fmt:message key="RLT01002A.venueNm"/>"
	   		, "<fmt:message key="RLT01002A.venueGradCD"/>"
	   		, "<fmt:message key="RLT01002A.segmentCD"/>"
	   		, "<fmt:message key="RLT01002A.subSegmentCD"/>"
	   		, "<fmt:message key="RLT01002A.addrTpCD1"/>"
	   		, "<fmt:message key="RLT01002A.addrTpCD2"/>"
	   		, "<fmt:message key="RLT01002A.tpayAmt"/>"
	   		, "<fmt:message key="RLT01002A.resultTQty"/>"
	   		, "<fmt:message key="RLT01002A.avgUnitAmt"/>"
	   		, "<fmt:message key="RLT01002A.resultTpCDName"/>"
	   		, "<fmt:message key="RLT01002A.targetRate"/>"
	   		, "<fmt:message key="RLT01002A.payRate"/>"
	   		, "<fmt:message key="RLT01002A.excPayTpCDName"/>"
	   		, "<fmt:message key="RLT01002A.gsvRate"/>"
	   		, "<fmt:message key="RLT01002A.commt"/>"
	   		, "<fmt:message key="RLT01002A.payAmt_keyman"/>"
	   		, "<fmt:message key="RLT01002A.apprStateCD"/>"
		...
		table.setParameter("templateFile", "RLT01002A");
