						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="RLT02005A.eventYM" />
												</td>
												<td class="td-input">
				                                ...
				                                <td class="td-cond">
													<fmt:message key="RLT02005A.officeCD" />
												</td>
												<td class="td-input">
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0200501S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"gubun;eventYM;typeName;venueCount;resultQty;"+
...
// 	  	if (grid != null) grid.clear();
      	...
      	if (rowCount > 0) {
      		...
      		for (var i=0; i<rowCount; i++) {
      			...
      			if(table.getData(_col_all.seq,i) == "2") {
      				...
      				resultQty = formatNum(table.getData(_col_all.resultQty,i))+' '+'%';
      			...
      			if(table.getData(_col_all.eventYM,i) == $("#yearCD_S").val()+$("#monthCD_S").val()){
      				...
      				grid_all.getCellTemplate(_col_all.resultQty,i).setStyle("font-weight", "bold");
	...
	function Search_exception(fromMonth) {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0200502S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"gubun;eventYM;typeName;venueCount;resultQty;"+
...
// 	  	if (grid != null) grid.clear();
      	...
      	if (rowCount > 0) {
      		...
      		for (var i=0; i<rowCount; i++) {
      			...
      			if(table.getData(_col_exception.seq,i) == "2") {
      				...
      				resultQty = formatNum(table.getData(_col_exception.resultQty,i))+' '+'%';
      			...
      			if(table.getData(_col_all.eventYM,i) == $("#yearCD_S").val()+$("#monthCD_S").val()){
      				...
      				grid_exception.getCellTemplate(_col_exception.resultQty,i).setStyle("font-weight", "bold");
	...
	function Approval() {
		...
		for (var i=0; i<index.length; i++) {
			if (unformat(grid_all.getCellValue(_col_all.eventYM, index[i])) == $("#yearCD_S").val()+$("#monthCD_S").val()) {
		...
		if(!confirm("<fmt:message key="RLT02005A.msg1"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/rlt02/approvalResultVenue_new");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function Reject() {
		if(!confirm("<fmt:message key="RLT02005A.msg2"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/rlt02/rejectResultVenue_new");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "RLT0200501S");
		table.setParameter("fromMonth", fromMonth);
	  	table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("empID", "${params.empID}");
	  	table.setParameter("diffNo", $("#diffNo").val());
		...
		var headerText = [
			"",
			"<fmt:message key="RLT02005A.Period"/>"
			, "<fmt:message key="RLT02005A.Type"/>"
			, "<fmt:message key="RLT02005A.VenueNumber"/>"
			, "<fmt:message key="RLT02005A.Volume(ActualCase)"/>"
			, "<fmt:message key="RLT02005A.Volume(EU)"/>"
			, "<fmt:message key="RLT02005A.Incentive(KKRW)"/>"
			, "<fmt:message key="RLT02005A.CaserateperA/C(KKRW)"/>"
			, "<fmt:message key="RLT02005A.CaserateperEU(KKRW)"/>"
			, "<fmt:message key="RLT02005A.NSV(KKRW)"/>"
			, "<fmt:message key="RLT02005A.GP(KKRW)"/>"
			, "<fmt:message key="RLT02005A.GPrate(%)"/>"
		];
		
		table.setParameter("templateFile", "RLT02005A");
