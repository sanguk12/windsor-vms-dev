...
<script language="javascript">
	...
	****************************************/		
	...
	function Search() {		
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0101101S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	...
	function RequestApp() {
		...
		//승인상태가 등록일 경우만 가능
		if(grid.getCellText(_col.apprStateCD, 2) != "10") {
		...
		table2.setURL("${contextPath}/service/simpleAction/RLT0101102S");
		...
		if(table2.getData(0,0) != "0") {
			alert("<fmt:message key="RLT01011A.msg1"/>");
			return;
		}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/rlt01/requestAppResult");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function ClearRequestApp() {
		...
		//승인요청 경우에만 승인취소 가능
		for (var i=0; i<grid.getRowCount(); i++){
			if (grid.getCellText(_col.summary,i) == "Total" && grid.getCellText(_col.summaryN,i) == "Result"){
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/rlt01/clearRequestAppResult");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "RLT0101101S");	
		...
		table.setParameter("templateFile", "RLT01011A");
