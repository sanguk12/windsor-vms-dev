									...
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="RLT01001C.msg1" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
													<b><span id="payAmt_sum"></span></b>
													<input type="hidden" id="tpayAmt" name="tpayAmt" value="${params.tpayAmt }" />
				                                </td>
				                            </tr>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="RLT01001C.msg2" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
...
<script language="javascript">
	...
	function calculatePayAmt_sum() {
		...
		var tpayAmt = parseFloat(unformat($("#tpayAmt").val()));
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		//2014.08.13, taejoon.jang, 검증오류는 result작성기간 안따짐
		if("${params.resultWriteYN}" != "N") {
			getResultWriteYN();
		}
...
// 	  	if (grid_reg != null) grid_reg.clear();
	...
	function ValidationCheck() {
		...
		//result승인처리가 등록, 반려(10:등록, 40:반려)가 아닌경우에는 이동 불가
		var apprStateCD = "${params.apprStateCD}";
		//검증상태가 등록, 반려, 검증오류(10:등록, 40:반려, 50:검증오류)가 아닌경우에는 이동 불가
		var verifyStateCD = "${params.verifyStateCD}";
		
		if(apprStateCD!="" && apprStateCD!="10" && apprStateCD!="40") {
			alert("<fmt:message key="RLT01001C.msg4"><fmt:param value='"+venueCD+"' /></fmt:message>");
			isBoolean = false;
		}
		if(verifyStateCD!="" && verifyStateCD!="10" && verifyStateCD!="40" && verifyStateCD!="ERR") {
			alert("<fmt:message key="RLT01001C.msg5"><fmt:param value='"+venueCD+"' /></fmt:message>");
			isBoolean = false;
		}
		return isBoolean;
	}
	...
	function Add() {
		if(opener.$("#resultWriteYN").val() == "N") {
			alert("<fmt:message key="RLT01001C.msg3" />");
			return;
		}
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="RLT01001C.keymanID"/>"
		, "<fmt:message key="RLT01001C.keymanNm"/>"
		, "<fmt:message key="RLT01001C.birthDate"/>"
		, "<fmt:message key="RLT01001C.bankCDName"/>"
		, "<fmt:message key="RLT01001C.accountNo"/>"
		, "<fmt:message key="RLT01001C.pkeymanYN"/>"
	];
	...
	var columns_reg = [
		...
		, "<fmt:message key="RLT01001C.bankCDName"/>"
		, "<fmt:message key="RLT01001C.accountNo"/>"
		, "<fmt:message key="RLT01001C.payAmt"/>"
	];
	var grid_reg = null;
