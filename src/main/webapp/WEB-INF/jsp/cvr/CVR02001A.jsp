...
<script language="javascript">
	...
	****************************************/		
	...
	function Search() {		
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/CVR0200101S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
		...
		if (rowCount > 0) {
			...
			for (var i=0; i<rowCount; i++) {
				...
				if (table.getData(_col.apprStateCD,i) == "20"){
					grid.setCellEditable(false, _col.NGGSVC, i);
					grid.setCellEditable(false, _col.BJ, i);
				}
      		}
	...
	function RequestApp() {
		...
		var nggsvRns = new Array();
		var nggsvRcs = new Array();
		...
		if (!confirm("<fmt:message key="CVR02001A.msg04"/>")) return;
		
		for (var i=0; i<maxRow; i++) {
			if(grid.getCellValue(_col.checkbox, i)) {
				//등록,반려,승인 시 승인요청 가능
				if(grid.getCellText(_col.apprStateCD, i) == "10" || grid.getCellText(_col.apprStateCD, i) == "40" || grid.getCellText(_col.apprStateCD, i) == "50"){
					...
					nggsvRns[count] = grid.getCellText(_col.NGGSVN, i);
					nggsvRcs[count] = grid.getCellText(_col.NGGSVC, i);
					commts[count] = grid.getCellText(_col.BJ, i);
					
					count++;
				}else{
					alert("<fmt:message key="CVR02001A.msg01"/>");
					return ;
				}
			}
		}
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/cvr02/saveApprReq");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYm", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	...
	  	table.setParameter("nggsvRns", nggsvRns);
	  	table.setParameter("nggsvRcs", nggsvRcs);
	...
	function ClearRequestApp() {
		...
		if (!confirm("<fmt:message key="CVR02001A.msg05"/>")) return;
		...
		for (var i=0; i<maxRow; i++) {
			if(grid.getCellValue(_col.checkbox, i)) {
				if(grid.getCellText(_col.apprStateCD, i) == "20"){
					...
					alert("<fmt:message key="CVR02001A.msg03"/>");
					return ;
				}	
			}
		}
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/cvr02/saveApprReqCancel");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYm", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("rageSphereCDs", rageSphereCDs);
	...
	*****************************************************/
	...
	function createGrid() {
	    ...
	    grid.setCellEditable(false, _col.checkbox);
	    grid.setCellEditable(false, _col.rageSphereNM);
	    grid.setCellEditable(false, _col.officeNM);
	    grid.setCellEditable(false, _col.kindTpNM);
	    grid.setCellEditable(false, _col.NGGSVN);
	    grid.setCellEditable(false, _col.apprStateNM);	  
