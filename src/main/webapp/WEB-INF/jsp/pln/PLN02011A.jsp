...
<script language="javascript">
	...
	****************************************/		
	...
	function Search() {	
		...
		table1.setURL("${contextPath}/service/simpleAction/PLN0201100S");
		...
		if (rowCount > 0) {
			...
			for (var i=0; i<rowCount; i++) {
				...
				if(i != 1 && table1.getData(_col.summaryN,i) == "PLAN" ){
      	...
      	var paramQuery = "PLN0201101S"		//지점장
      	if ($("#apprLv").val() == "30")  paramQuery = "PLN0201103S";	//권역장
      	if ($("#apprLv").val() == "40")  paramQuery = "PLN0201105S";	//CM
		...
		if (rowCount > 0) {
			...
			for (var i=0; i<rowCount; i++) {
				...
				if(table.getData(_col2.summary,i) != "Total" && table.getData(_col2.summaryN,i) == "PLAN" ){
	...
	function Approval() {	
		...
		if ($("#apprLv").val() == "20"){
		    ...
		    //현재 PLAN NGSV는 지점별 NGSV 이하여야 함
			if (grid1.getCellValue(_col.NGGSV ,3) > ngsvLocal){
				alert("<fmt:message key="PLN02001A.msg11"/>");
				return;
			}
		
			if (grid1.getCellValue(_col.NGGSV ,5) > ngsvInter){
				alert("<fmt:message key="PLN02001A.msg11"/>");
				return;
			}		
		...
		for (var i=0; i<grid2.getRowCount(); i++){	
			//하위 레벨 직원의 승인요청(승인) 확인
			if (grid2.getCellValue(_col2.summary ,i) == "Total" && grid2.getCellValue(_col2.summaryN ,i) == "PLAN"){
		...
		//승인 처리
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pln02/approvalPlan");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	...
	function Reject() {
		...
		if ($("#apprLv").val() == "30" || $("#apprLv").val() == "40"){
		...
		for(var i=0; i<grid2.getRowCount(); i++) {
			if (grid2.getCellValue(_col2.checkbox, i)) {
				...
				if(grid2.getCellText(_col2.apprStateCD, i+1) == "50") {
					empIDs[empIDCnt] = grid2.getCellText(_col2.empID, i+1);
					empIDCnt++;
				}else{
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pln02/rejectPlan");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function Excel() {
		...
		table.setParameter("templateFile", "PLN02011A");
	...
	function checkAppr(){
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/simpleAction/PLN0201108S");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM" , $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.userID}");
	...
	*****************************************************/
	...
	function createGrid() {
	    ...
	    if ($("#apprLv").val() == "20"){
		    //grid2.setCellTemplate(new AW.Templates.CheckBox, _col2.checkbox);
