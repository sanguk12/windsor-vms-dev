												...
												<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
							        				...
							        				<tbody>
			                            				...
			                            				<tr>
															...
															<td class="td-cond-report">
																<fmt:message key="PLN01007A.GPRate" />
															</td>
														</tr>
														<tr>	
															<td class="td-report-rborder">
...
<style type="text/css">
    ...
    #grid_prd .aw-column-7 {width: 90px; text-align:right;}
    #grid_prd .aw-column-8 {width: 90px; text-align:right;}
    #grid_prd .aw-column-9 {width: 90px; text-align:right;}
    #grid_prd .aw-column-10 {width: 60px; text-align:right;}
...
<script language="javascript">
	...
	function calculateNsv(row) {
		var planQty = parseFloat(unformatNum(grid_prd.getCellText(_col_prd.planQty, row)));
		...
		var payAmt = parseFloat(unformatNum(grid_prd.getCellText(_col_prd.payAmt, row)));
		...
		gsv = gsv_prd * planQty;
		duty = duty_prd * planQty;
		wsdc = wsdc_prd * planQty;
		cogs = cogs_prd * planQty;

		nsv = Math.round((gsv - duty - wsdc - payAmt) * 10) / 10;
	...
	//당월Plan 총nsv계산
	function calculateTnsv() {
		var tNsv = 0.0;
		var tGp = 0.0;
		var tGpRate = 0.0;
		for(var i=0; i<grid_prd.getRowCount(); i++) {
			tNsv += parseFloat(unformatNum(grid_prd.getCellText(_col_prd.nsv, i)));
			tGp += parseFloat(unformatNum(grid_prd.getCellText(_col_prd.gp, i)));
		}
		
		tNsv = Math.round(tNsv * 10) / 10;
		tGp = Math.round(tGp * 10) / 10;
		tGpRate = Math.round((tGp / tNsv) * 1000) / 10;

		$("#tNsv").text(formatNum(tNsv));
		$("#tGp").text(formatNum(tGp));
		$("#tGpRate").text(formatNum(tGpRate));
	}
	...
	function calculateGsvRate() {
		...
		$("#gsvRate").text(Math.round(gsv * 10) / 10);
	...
	function Search_dtl() {
		var table = new AW.XML.Table;
	  	...
	  	table.setParameter("outParamKey",
	  			...
	  			"apprStateCDName;tNsv;tGp;tGpRate;apprStateCD;"+
	  			"commt;programCD;");
	  	table.setParameter("format",
	  			"str;str;str;str;num;"+
	  			"num;num;num1;num;str;"+
	  			"str;num1;num1;num1;str;"+
	  			"str;str;");	  	
      	...
      	if (rowCount > 0) {
    		...
    		table_t.setRequestMethod("POST");
         	...
         	if(table_t.getCount() > 0) {
      		...
      		$("#tNsv").text(table.getData(_col_dtl.tNsv, 0));
      		$("#tGp").text(table.getData(_col_dtl.tGp, 0));
      		$("#tGpRate").text(table.getData(_col_dtl.tGpRate, 0));
	...
	function Search_prd() {
		var table = new AW.XML.Table;
	  	...
	  	table.setParameter("outParamKey",
	  			...
	  			"gpRate;prdCD;gsv_prd;duty_prd;wsdc_prd;"+
	  			"cogs_prd;");
	  	table.setParameter("format",
	  			"str;str;num;str;num;"+
	  			"num;num;num;num1;num1;"+
	  			"num1;str;num1;num1;num1;"+
				"num1;");
	  	table.setParameter("eventYM", "${params.eventYM}");
	  	table.setParameter("venueCD", "${params.venueCD}");
      	...
      	if (rowCount > 0) {
      		...
      		for (var i=0; i<rowCount; i++) {
      			data[i] = [
					...
					, table.getData(_col_prd.nsv,i)
					, table.getData(_col_prd.gp,i)
					, table.getData(_col_prd.gpRate,i)
