						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="PLN01004A.eventYM" />
												</td>
...
<!-- 				                                <td class="td-cond-required"> -->
				                                <td class="td-cond">
													<fmt:message key="PLN01004A.subSegmentCD" />
												</td>
												<td class="td-input">
                	...
                	<td height="1">
                		<table border="0" cellspacing="0" cellpadding="0" width="100%" >
			        		...
			        		<tbody>
			        			<tr>
									<td class="tit_text01">
										<fmt:message key="PLN01004A.planWriteDate" />
									</td>
									<td class="tit_text01">
...
// 		if($("#subSegmentCD_S").val() == "" ) {
// 			alert("<fmt:message key="PLN01004A.msg4"/>");
// 			return;
...
// 	  	if (grid != null) grid.clear();
	...
	function Search_pln() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PLN0100401S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;activityCDName;venueCD;venueNm;"+
		...
		for(var i=0; i < index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(table.getData(0,0) == "Y") {
					...
					grid_pln.setCellText("선지원FollowUp", _col_pln.activityCDName, maxRow_pln-1);
					...
					grid_pln.setCellText("<fmt:message key="PLN01004A.activityCD_10" />", _col_pln.activityCDName, maxRow_pln-1);
...
// 				var apprStateCDName = grid_pln.getCellText(_col_pln.apprStateCDName, index_pln[i]);
				if(apprStateCD=="50") {
					alert("<fmt:message key="PLN01004A.msg1"><fmt:param value='"+venueCD+"' /></fmt:message>");
					return;
				}
	...
	function Save() {
		...
		for(var i=0; i<maxRow_pln; i++) {
			if(grid_pln.getCellText(_col_pln.activeFlg, i)!="D") {
				if (grid_pln.getCellText(_col_pln.activityCD, i) == "") {alert("<fmt:message key="PLN01004A.msg2" />"); setCellFocus(grid_pln, _col_pln.activityCDName, i); return;}
			}
		}

		var venueCD = new Array();
		var programCD = new Array();
		var activityCD = new Array();
		var activeFlg = new Array();
		var count = 0;
		for (var i=0; i<maxRow_pln; i++) {
			venueCD[count] = grid_pln.getCellText(_col_pln.venueCD, i);
			programCD[count] = grid_pln.getCellText(_col_pln.programCD, i);
			activityCD[count] = grid_pln.getCellText(_col_pln.activityCD, i);
			activeFlg[count] = grid_pln.getCellText(_col_pln.activeFlg, i);
			count++;
		}
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pln01/savePlanBrand");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
		...
		table.setParameter("programCD", programCD);
		table.setParameter("activityCD", activityCD);
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "PLN0100401S");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
		table.setParameter("venueGradCD", $("#venueGradCD_S").val());
	  	table.setParameter("segmentCD", $("#segmentCD_S").val());
		...
		var headerText = [
			"<fmt:message key="PLN01004A.activity"/>"
       		, "<fmt:message key="PLN01004A.venueCD"/>"
			, "<fmt:message key="PLN01004A.venueNm"/>"
			, "<fmt:message key="PLN01004A.venueGradCD"/>"
			, "<fmt:message key="PLN01004A.subSegmentCD"/>"
			, "<fmt:message key="PLN01004A.addrTpCD2"/>"
		];
		
		table.setParameter("templateFile", "PLN01004A");
	...
	function CopyLastMonPlanBusi() {
		if(!confirm("<fmt:message key="PLN01004A.msg3"/>")) {
			return;
		}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pln01/copyBefMonPlanBrand");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	*****************************************************/
	...
	function createGrid_pln() {
		...
		grid_pln.setPopupTemplate(function(col, row) {
			...
			if (col == _col_pln.activityCDName) {
	        	var table = new AW.XML.Table;
	        	table.setURL("${contextPath}/service/simpleAction/SYS0001S");
	        	table.setAsync(false);
	        	table.setRequestMethod("POST");
