			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
            	<tr>
                    <td height="1">
						...
						<input type="hidden" id="appSrcB" name="appSrcB" value="${contextPath}/service/simpleCommand/?mnuGrpID=${params.mnuGrpID}&pgmID=${params.pgmID}&viewID=AD02007B" />
						<input type="hidden" id="appSrcC" name="appSrcC" value="${contextPath}/service/simpleCommand/?mnuGrpID=${params.mnuGrpID}&pgmID=${params.pgmID}&viewID=AD02007C" />
...
<style type="text/css">
    ...
    #grid_srch .aw-column-12 {width: 100px; text-align:center;}
    #grid_srch .aw-column-13 {width: 60px; text-align:center;}
    #grid_srch .aw-column-14 {width: 100px; text-align:center;}
    #grid_srch .aw-column-15 {width: 60px; text-align:center;}
    #grid_srch .aw-column-16 {width: 100px; text-align:left;}
    #grid_srch .aw-column-17 {width: 60px; text-align:center;}
    #grid_srch .aw-column-18 {width: 70px; text-align:center;}
    #grid_srch .aw-column-19 {width: 120px; text-align:left;}
    #grid_srch .aw-column-20 {width: 80px; text-align:center;}
    #grid_srch .aw-column-21 {width: 80px; text-align:center;}
    #grid_srch .aw-column-22 {width: 80px; text-align:center;}
...
<script language="javascript">
	...
	function firstInit() {
		...
		$("#apprStateCD_S option:eq(5)").prop("selected", true); //승인요청		
	...
	function checkContractTmpList() {
		...
		table.setParameter("adContractDivCD", "20"); //계약구분 : 변경
		table.request();
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search_srch() {
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/simpleAction/AD0200701S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	...
	function Search_dtl() {
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/simpleAction/AD0200718S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"contractDivCD;venueCD;venueNM;startDT;endDT;"+
    	...
    	var rowCount = table.getCount();
      	...
      	if (rowCount > 0) {
      		...
      		for (var i = 0; i < rowCount; i++) {
				...
				if (adChgFlg == 'Y') {
					grid_dtl.getCellTemplate(_col_dtl.apprStateName, i).setStyle("background", "#FFFF00");
		...
		if(grdDivCD == "srch") {
      	...
      	//AD계약수정 수정 목록 Grid
			...
			venueCD = grid_dtl.getCellText(_col_dtl.venueCD,i);
      		...
      		adChgFlg = grid_dtl.getCellText(_col_dtl.adChgFlg,i);
      		if(adChgFlg == "Y"){
      			apprTpIDEx = "000010";
      		} else{
      			// apprTpIDEx = (apprExpc == "2") ? myADApprExID : myApprID;
