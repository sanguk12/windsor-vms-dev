					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
		            	<tr>
		                    <td height="1">
								...
								<input type="hidden" id="appSrc" name="appSrc" value="${contextPath}/service/simpleCommand/?mnuGrpID=${params.mnuGrpID}&pgmID=${params.pgmID}&viewID=MIL01001B" />
							</td>
						</tr>
						<tr>
							<td height="1">
								        		...
								        		<tbody>
						                            <tr>
														<td class="td-cond-required">
															<fmt:message key="MIL01001A.contractDate" />
														</td>
														<td class="td-input">
														<!-- 
						                                ...
						                                <td class="td-cond">
															<fmt:message key="MIL01001A.payRequestDate" />
														</td>
														<td class="td-input">
															<nis:calendar id="payRequestDate_S" />
						                                </td>
						                                <td class="td-cond">
															<fmt:message key="MIL01001A.mileageStateCD" />
														</td>
														<td class="td-input">
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/MIL0100101S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"contractDateTo;contractMonthCt;msTargetRate;tcontractQty;"+
...
// 	  	if (grid != null) grid.clear();
	...
	function RequestApp() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.mileageStateCD, index[i]) != "10") {
					alert("<fmt:message key="MIL01001A.msg1"/>");
					return;
				}
				...
				if(addContractAmt != keymanAmt) {
					...
					alert("<fmt:message key="MIL01001A.msg2"/>");
					return;
				}
				*/
			}
		...
		if(!confirm("<fmt:message key="MIL01001A.msg3"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/mil01/requestPayMileage");
		table.setAsync(false);
		table.setRequestMethod("POST");
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.mileageStateCD, index[i]) != "20") {
					alert("<fmt:message key="MIL01001A.msg4"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="MIL01001A.msg5"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/mil01/clearRequestPayMileage");
		table.setAsync(false);
		table.setRequestMethod("POST");
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "MIL0100106S");
		table.setParameter("empID", "${params.empID}");
	  	//table.setParameter("contractDateFrom", unformat($("#contractDateFrom_S").val()));
	  	//table.setParameter("contractDateTo", unformat($("#contractDateTo_S").val()));
	  	table.setParameter("contractDateFrom", $("#yearCDFrom_S").val()+$("#monthCDFrom_S").val());
		...
		var headerText = [
			"<fmt:message key="MIL01001A.mileageID"/>"
			, "<fmt:message key="MIL01001A.venueCD"/>"
			, "<fmt:message key="MIL01001A.venueNm"/>"
			, "<fmt:message key="MIL01001A.venueGradCD"/>"
			, "<fmt:message key="MIL01001A.segmentCD"/>"
			, "<fmt:message key="MIL01001A.subSegmentCD"/>"
			, "<fmt:message key="MIL01001A.addrTpCD1"/>"
			, "<fmt:message key="MIL01001A.addrTpCD2"/>"
			, "<fmt:message key="MIL01001A.contractDateFrom"/>"
			, "<fmt:message key="MIL01001A.contractDateTo"/>"
			, "<fmt:message key="MIL01001A.contractMonthCt"/>"
			//, "<fmt:message key="MIL01001A.apAmt"/>"
			, "<fmt:message key="MIL01001A.msTargetRate"/>"
			, "<fmt:message key="MIL01001A.tcontractQty"/>"
			, "<fmt:message key="MIL01001A.gsvAmt"/>"
			, "<fmt:message key="MIL01001A.gsvRate"/>"
			, "<fmt:message key="MIL01001A.contractAmt"/>"
			, "<fmt:message key="MIL01001A.contractTaxAmt"/>"
			, "<fmt:message key="MIL01001A.addContractAmt"/>"
			, "<fmt:message key="MIL01001A.addContractTaxAmt"/>"
			, "<fmt:message key="MIL01001A.keymanAmt"/>"
			, "<fmt:message key="MIL01001A.payRequestDate"/>"
			, "<fmt:message key="MIL01001A.mileageStateCD"/>"
			, "<fmt:message key="MIL01001A.mileagePayTPCD"/>"
			, "<fmt:message key="MIL01001A.commt"/>"
		...
		table.setParameter("templateFile", "MIL01001A");
	...
	*****************************************************/
	var columns = [
		...
		, "<fmt:message key="MIL01001A.mileageStateCD"/>"
		, "<fmt:message key="MIL01001A.mileagePayTPCD"/>"
		//, "<fmt:message key="MIL01001A.transferDate"/>"
		, "<fmt:message key="MIL01001A.commt"/>"
	];
