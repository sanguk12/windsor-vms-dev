...
<style type="text/css">
    ...
    #grid .aw-header-1 .aw-column-22 {text-align:left;}  
    #grid .aw-header-1 .aw-column-23 {text-align:left;}  
    #grid .aw-header-1 .aw-column-24 {text-align:left;}  
    ...
    #grid .aw-column-8 {width: 150px; text-align:left;}    
    ...
    #grid .aw-column-14 {width: 110px; text-align:left;}
    #grid .aw-column-15 {width: 100px; text-align:left;}
    #grid .aw-column-16 {width: 100px; text-align:left;}
    #grid .aw-column-17 {width: 100px; text-align:left;}
    #grid .aw-column-18 {width: 100px; text-align:left;}
    ...
    #grid .aw-column-38 {width: 100px; text-align:left;}
...
// 			}
	...
	function PagingSearch() {
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/simpleAction/MDM0700301S");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("outParamKey",
				"checkbox;requestDate;apprStateCDName;requestStateCDName;venueCD;"+                           
				"venueNm;reason1CDNm;reason2;territoryCDName;phoneNo;"+             
				...
				"segmentCDName;subSegmentCDName;reserveSegmentCDName;ageingCDName;kegYN;"+                        
				"activeYN;ws1SapCD;ws1SapCDName;branch1;ws2SapCD;"+                           
				"ws2SapCDName;branch2;ws3SapCD;ws3SapCDName;branch3;"+
				"ws4SapCD;ws4SapCDName;branch4;commt;apprCommt;"+
				"apprStateCD;requestStateCD;levelNo;lapprlevelno;lastApprYN;"+
				"apprTpID;empID;rowNum;"
	  			);
...
// 	  	if (grid != null) grid.clear();
      	...
      	if (rowCount > 0) {
			...
			table.setURL("${contextPath}/service/simpleAction/MDM0700302S");
			table.setAsync(false);
			table.setRequestMethod("POST");
	...
	function Approval() {
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/mdm07/approvalRmTmpVenue");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("empID", "${params.empID}");
	...
	function Reject() {
		...
		table.setURL("${contextPath}/service/mdm07/rejectRmTmpVenue");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("empID", "${params.empID}");
		table.setParameter("venueCDs", venueCDs);
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "MDM0700306S");
		table.setParameter("empID", "${params.empID}");
		table.setParameter("requestDateFrom", convertDate($("#requestDateFrom_S").val()));
		table.setParameter("requestDateTo", convertDate($("#requestDateTo_S").val()));
		table.setParameter("apprStateCD", $("#apprStateCD_S").val());
		...
		table.setParameter("templateFile", "MDM07003A");
...
// 		updatePrintYN();
	...
	//엑셀	
	function OutletSellingAreaDeactivationExcel() {
		var index = grid.getRowIndices();
		var venueCDs = new Array();
		var venueCD = "";
		var countChk = 0;
...
// 		if(!confirm("<fmt:message key="MDM07003A.msg1"/>")) {
//        		return;
//        	}
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				if(grid.getCellText(_col.requestStateCD, index[i]) != "50") {
					alert("상태가 승인인 것만 엑셀 다운로드를 하실 수 있습니다.");
					return;
				}
			}
		}
...
//		printChk(venueCDs, venueCD);
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "MDM0700304S");
		table.setParameter("empID", "${params.empID}");
		table.setParameter("venueCDs", venueCDs);
		table.setParameter("venueCD", venueCD);
	  	
		var headerText = [
			"<fmt:message key="MDM07003A.sellingAreaID"/>"
			,"<fmt:message key="MDM07003A.sellingAreaRef"/>"
			,"<fmt:message key="MDM07003A.sellingAreaStatus"/>"
			,"<fmt:message key="MDM07003A.outletID"/>"
			,"<fmt:message key="MDM07003A.outletRef"/>"
			,"<fmt:message key="MDM07003A.outletStatus"/>"
		];
		
		table.setParameter("templateFile", "Outlet_Selling_Area_Deactivation");
		table.setParameter("headerText", headerText);
		table.request();
		...
		if (table.getCount() > 0) {
			if (table.getData(0,0) == "S") {
			    form.action = "${contextPath}/service/simpleExport/download/?"+
			    		"tempFile="+table.getData(3,0)+
			    		"&downloadFile=Outlet_Selling_Area_Deactivation";
			    form.target = "_self";
			    form.submit();
			}
		}
	...
	//엑셀 출력여부
	function printChk(venueCDs, venueCD) {
		var table = new AW.XML.Table;
