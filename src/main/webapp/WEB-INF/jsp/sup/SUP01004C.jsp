                ...
                <tr>
					<td height="1" valign="top">
						<table border="0" cellspacing="0" cellpadding="0" width="100%" height="100%">
							...
							<tr>
								<td height="1">
									<fmt:message key="SUP01002C.addPayRequestDate" /> : <nis:calendar id="payRequestDate" cssClass="input-readonly" readOnly="yes" />
								</td>
							</tr>
							<tr>
								<td height="1">
									...
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
						        			<tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="SUP01002C.addAmt_sum" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
													<b><span id="addAmt_sum"></span></b>
				                                </td>
				                            </tr>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="SUP01002C.mainAddAmt" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
													<b><span id="mainAddAmt"></span></b>
				                                </td>
				                            </tr>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="SUP01002C.addAmt_diff" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
													<b><span id="addAmt_diff"></span></b>
				                                </td>
				                            </tr>
			                            </tbody>
			                        </table>
...
<script language="javascript">
	...
	$(document).ready(function() {
		...
		window.onload = function() {
			...
			$("#mainAddAmt").text(formatNum("${params.mainAddAmt }"));
		...
		$("#payRequestDate").keypress(function(){
			if (event.keyCode==13){
				return false;
			}
		});
	...
	function calculatePayAmt_sum() {
		...
		$("#addAmt_sum").text(formatNum(payAmt));
		
		var mainAddAmt = parseFloat(unformat($("#mainAddAmt").text()));
		var addAmt_sum = parseFloat(unformat($("#addAmt_sum").text()));
		var addAmt_diff = 0;
		addAmt_diff = mainAddAmt - addAmt_sum;
		$("#addAmt_diff").text(formatNum(addAmt_diff));
	}
	...
	****************************************/
	...
	function Search() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/SUP0100104S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;keymanID;keymanNm;birthDate;bankCDName;"+
	  			"accountNo;pkeymanYN;bankCD;");
	  	table.setParameter("format",
	  			"str;str;str;str;str;"+
	  			"str;str;str;");
	  	table.setParameter("supportID", "${params.supportID}");
...
// 	  	if (grid != null) grid.clear();
	...
	var _col_reg = {
		...
		, bankCD : _i_reg++
		, activeFlg : _i_reg++
		, payRequestDate : _i_reg++
    };
	function Search_reg() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/SUP0100205S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;keymanID;keymanNm;birthDate;bankCDName;"+
	  			"accountNo;payAmt;pkeymanYN;bankCD;activeFlg;payRequestDate;");
	  	table.setParameter("format",
	  			"str;str;str;str;str;"+
...
// 	  	if (grid_reg != null) grid_reg.clear();
      	...
      	if (rowCount > 0) {
      		...
      		$("#payRequestDate").val(formatDate(table.getData(_col_reg.payRequestDate, 0)));
	...
	function Add() {
		...
		//선지원정보의 상태가 등록, 지급반려, 지급보류가 아닌경우에는 추가 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="" && supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01002C.msg1"/>");
			return;
		}
	...
	function Remove() {
		...
		//선지원정보의 상태가 등록, 지급반려가 아닌경우에는 삭제 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="" && supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01002C.msg1"/>");
			return;
		...
		for(var i=0; i<index_reg.length; i++) {
			if (grid_reg.getCellValue(_col_reg.checkbox, index_reg[i])) {
				...
				var table = new AW.XML.Table;
			  	...
			  	table.setRequestMethod("POST");
			  	table.setParameter("outParamKey",
			  			"bankCDName;accountNo;pkeymanYN;bankCD;");
			  	table.setParameter("format",
			  			"str;str;str;str;");
				...
				if(table.getCount() > 0) {
					...
					grid.setCellText(table.getData(3,0), _col.bankCD, maxRow-1);
				}
				else {
	...
	function Save() {
		//선지원정보의 상태가 등록, 지급반려가 아닌경우에는 저장 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="" && supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01002C.msg1"/>");
			return;
		...
		if(unformat($("#addAmt_diff").text()) != "0") {
			alert("<fmt:message key="SUP01002C.msg2"/>");
			return;
		}
		if($("#payRequestDate").val() == "") {
			alert("<fmt:message key="SUP01002C.msg3"/>");
			return;
		}
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/sup01/saveSupportKeyman_add");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("supportID", "${params.supportID}");
		...
		table.setParameter("addAmt", unformat($("#addAmt_sum").text()));
		
		var addAmt = parseFloat(unformat($("#addAmt_sum").text()));
		var addTaxAmt = 0;
		addTaxAmt = Math.round(addAmt * (1+0.0352));
		table.setParameter("addTaxAmt", addTaxAmt);
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="SUP01002C.keymanID"/>"
		, "<fmt:message key="SUP01002C.keymanNm"/>"
		, "<fmt:message key="SUP01002C.birthDate"/>"
		, "<fmt:message key="SUP01002C.bankCDName"/>"
		, "<fmt:message key="SUP01002C.accountNo"/>"
		, "<fmt:message key="SUP01002C.pkeymanYN"/>"
	];
	...
	var columns_reg = [
		...
		, "<fmt:message key="SUP01002C.bankCDName"/>"
		, "<fmt:message key="SUP01002C.accountNo"/>"
		, "<fmt:message key="SUP01002C.payAmt"/>"
	];
	var grid_reg = null;
