									...
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="SUP01001C.msg1" />
												</td>
				                            ...
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="SUP01001C.msg2" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
...
// 	  	if (grid != null) grid.clear();
	...
	function Search_reg() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/SUP0100105S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;keymanID;keymanNm;birthDate;bankCDName;"+
...
// 	  	if (grid_reg != null) grid_reg.clear();
	...
	function Add() {
		if(opener.$("#resultWriteYN").val() == "N") {
			alert("<fmt:message key="SUP01001C.msg3" />");
			return;
		}
		...
		//선지원정보의 상태가 등록, 지급반려, 보류가 아닌경우에는 추가 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01001C.msg4" />");
			return;
		}
	...
	function Remove() {
		...
		//선지원정보의 상태가 등록, 지급반려, 보류가 아닌경우에는 삭제 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01001C.msg4" />");
			return;
	...
	function Save() {
		//선지원정보의 상태가 등록, 지급반려, 보류가 아닌경우에는 저장 불가(10:등록, 40:지급반려, 50:지급보류)
		var supportStateCD = opener.$("#supportStateCD").val();
		if(supportStateCD!="10" && supportStateCD!="40" && supportStateCD!="50") {
			alert("<fmt:message key="SUP01001C.msg4" />");
			return;
		...
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/sup01/saveSupportKeyman");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("supportID", "${params.supportID}");
		table.setParameter("payTpCD", $("#payTpCD").val());
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="SUP01001C.keymanID"/>"
		, "<fmt:message key="SUP01001C.keymanNm"/>"
		, "<fmt:message key="SUP01001C.birthDate"/>"
		, "<fmt:message key="SUP01001C.bankCDName"/>"
		, "<fmt:message key="SUP01001C.accountNo"/>"
		, "<fmt:message key="SUP01001C.pkeymanYN"/>"
	];
	...
	var columns_reg = [
		...
		, "<fmt:message key="SUP01001C.bankCDName"/>"
		, "<fmt:message key="SUP01001C.accountNo"/>"
		, "<fmt:message key="SUP01001C.payAmt"/>"
	];
	var grid_reg = null;
