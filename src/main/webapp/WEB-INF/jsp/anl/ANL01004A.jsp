                    			...
                    			<td class="tb-border">
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
				                            ...
				                            <tr>
				                            	<td class="td-cond">
													<fmt:message key="ANL01004A.rageSphereCD" />
												</td>
												<td class="td-input">
				                                ...
				                                <td class="td-cond">
													<fmt:message key="ANL01004A.officeCD" />
												</td>
												<td class="td-input">
...
<script type="text/javascript">
	...
	****************************************/
	...
	function PagingSearch() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/ANL0100401S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"rageSphereCDName;officeCDName;incntTpCDName;kindTpCD5Name;kindTpCD2Name;"+
	  			"venueCount;payAmt;resultQty_eu;euRate;guidAmtExp;"+
	  			"sdxTpCD;targetRate;excPayTpCD;gp;");
...
// 	  	if (grid != null) grid.clear();
		...
		if (rowCount > 0) {
			...
			for (var i=0; i<rowCount; i++) {
				data[i] = [
					...
					, table.getData(_col.resultQty_eu,i)
					, table.getData(_col.euRate,i)
					, table.getData(_col.guidAmtExp,i)
      		...
      		grid.setFooterText(table.getData(_col.resultQty_eu,rowCount-1), _col.resultQty_eu);
      		grid.setFooterText(table.getData(_col.euRate,rowCount-1), _col.euRate);
      		grid.setFooterText(table.getData(_col.guidAmtExp,rowCount-1), _col.guidAmtExp);
      		grid.setFooterText(table.getData(_col.sdxTpCD,rowCount-1), _col.sdxTpCD);
      		grid.setFooterText(table.getData(_col.targetRate,rowCount-1), _col.targetRate);
      		grid.setFooterText(table.getData(_col.excPayTpCD,rowCount-1), _col.excPayTpCD);
		}
      	grid.refresh();
	}
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "ANL0100401S");
		table.setParameter("rageSphereCD_S", "${params.rageSphereCD}");
	  	table.setParameter("officeCD_S", "${params.officeCD}");
	  	table.setParameter("teamCD_S", "${params.teamCD}");
	  	table.setParameter("territoryCD", "${params.territoryCD}");
		...
		var headerText = [
			"<fmt:message key="ANL01004A.rageSphereCD"/>"
			, "<fmt:message key="ANL01004A.officeCD"/>"
			, "<fmt:message key="ANL01004A.incntTpCDName"/>"
			, "<fmt:message key="ANL01004A.kindTpCD5Name"/>"
			, "<fmt:message key="ANL01004A.kindTpCD2Name"/>"
			, "<fmt:message key="ANL01004A.venueCount"/>"
			, "<fmt:message key="ANL01004A.payAmt"/>"
			, "<fmt:message key="ANL01004A.resultQty_eu"/>"
			, "<fmt:message key="ANL01004A.euRate"/>"
			, "<fmt:message key="ANL01004A.guidAmtExp"/>"
			, "<fmt:message key="ANL01004A.sdxTpCD"/>"
			, "<fmt:message key="ANL01004A.targetRate"/>"
			, "<fmt:message key="ANL01004A.excPayTpCD"/>"
		];
		
		table.setParameter("templateFile", "ANL01004A");
