									...
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="PAY04012B.msg1" />
												</td>
				                            ...
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="PAY04012B.msg2" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
...
<script language="javascript">
	...
	function calculatePayAmt_sum() {
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0401203S");
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0401204S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
...
// 	  	if (grid != null) grid.clear();
	...
	function Search_reg() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0401205S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;keymanID;keymanNm;birthDate;bankCDName;"+
...
// 	  	if (grid_reg != null) grid_reg.clear();
	...
	function Add() {
		if(opener.$("#resultWriteYN").val() == "N") {
			alert("<fmt:message key="PAY04012B.msg3" />");
			return;
		}
		...
		var mileageID = "${params.mileageID}";
		...
		if(payStateCD!="" && payStateCD!="10" && payStateCD!="40" && payStateCD!="50") {
			alert("<fmt:message key="PAY04012B.msg4"><fmt:param value='"+mileageID+"' /></fmt:message>");
			return;
		}
	...
	function Save() {
		...
		var payStateCD = "${params.payStateCD}";
		if(payStateCD!="" && payStateCD!="10" && payStateCD!="40" && payStateCD!="50") {
			alert("<fmt:message key="PAY04012B.msg5"><fmt:param value='"+mileageID+"' /></fmt:message>");
			return;
		}
		//업소지급액이 0이 아니면 저장불가
		var payAmt_diff = Number(unformat($("#payAmt_diff").text())); 
		if(payAmt_diff != 0) {
			alert("<fmt:message key="PAY04012B.msg6"/>");
			return;
		}
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="PAY04012B.keymanID"/>"
		, "<fmt:message key="PAY04012B.keymanNm"/>"
		, "<fmt:message key="PAY04012B.birthDate"/>"
		, "<fmt:message key="PAY04012B.bankCDName"/>"
		, "<fmt:message key="PAY04012B.accountNo"/>"
		, "<fmt:message key="PAY04012B.pkeymanYN"/>"
	];
	...
	var columns_reg = [
		...
		, "<fmt:message key="PAY04012B.bankCDName"/>"
		, "<fmt:message key="PAY04012B.accountNo"/>"
		, "<fmt:message key="PAY04012B.payAmt"/>"
	];
	var grid_reg = null;
