	...
	function PagingSearch() {
		var table = new AW.XML.Table;
	  	...
	  	table.setParameter("outParamKey",
	  			...
	  			"payApprDate;transferDueDate;payCommt;mileagePayTPCDName;payStateCDName;payStateCD;"+
	  			"rowNum;totalCount;");
...
// 	  	if (grid != null) grid.clear();
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(payStateCD != "10" && payStateCD != "40" && payStateCD != "50") {
					alert("<fmt:message key="PAY04012A.msg1"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="PAY04012A.msg2"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay04/applicationPayMileage");
		table.setAsync(false);
		table.setRequestMethod("POST");
		//table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		//table.setParameter("diffNo", $("#diffNo").val());
	...
	function ReVerifyReq() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				var payStateCD = grid.getCellText(_col.payStateCD, index[i]);
				if(payStateCD != "10" && payStateCD != "40" && payStateCD != "50") {
					alert("<fmt:message key="PAY04012A.msg3"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="PAY04012A.msg4"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay04/reVerifyReqMileage");
		table.setAsync(false);
		table.setRequestMethod("POST");
		//table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		//table.setParameter("diffNo", $("#diffNo").val());
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(payStateCD != "30" && payStateCD != "20" && payStateCD != "10") {
					alert("<fmt:message key="PAY04012A.msg5"/>");
					return;
				}
	...
	function PayDueDateSave(){
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				var payStateCD = grid.getCellText(_col.payStateCD, index[i]);
				if(grid.getCellText(_col.transferDueDate, index[i]) == "") {
					alert("<fmt:message key="PAY04012A.msg7"/>");
					setCellFocus(grid, _col.transferDueDate, index[i]);
					return;
				}
			}
		...
		if(!confirm("<fmt:message key="PAY04012A.msg6"/>")) {
       		return;
       	}
	...
	*****************************************************/
	var columns = [
		...
		, "<fmt:message key="PAY04012A.mileagePayTPCDName"/>"
