			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" height="100%">
						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="PAY02002A.eventYM" />
												</td>
												<td class="td-input">
											...
											<tr>
												<td class="td-cond">
													<fmt:message key="PAY02002A.segmentCD" />
												</td>
												<td class="td-input">
				                            ...
				                            <tr>
				                            	<td class="td-cond">
													<fmt:message key="PAY02002A.rageSphereCD" />
												</td>
												<td class="td-input">
				                            ...
				                            <tr>    
												<td class="td-cond">
													<fmt:message key="PAY02002A.apprStateCD" />
												</td>
												<td class="td-input">
													<select id="apprStateCD_S" name="apprStateCD_S"></select>
				                                </td>
				                                <td class="td-cond">
													<fmt:message key="PAY02002A.sdxTpCD" />
												</td>
				...
				<tr>
					<td height="1">
						...
						<input type="hidden" id="apprTpID" name="apprTpID" value="000002" /><!-- 검증승인라인 -->
					</td>
				</tr>
			</table>
		</form>
...
<style type="text/css">
    ...
    .aw-column-33 {width: 100px; text-align:center; }
...
<script type="text/javascript">
	...
	****************************************/
	...
	function PagingSearch() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0200201S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"apprCommt;verifyErrCommt;apprStateCDName_emp;verifyStateCDName;verifyStateCD;"+
	  			"apprStateCD_emp;lapprLevelNo;levelNo;lastApprYN;prdCD;"+
	  			"sdxTpCD;sdxYN;rowNum;totalCount;");
...
// 	  	if (grid != null) grid.clear();
      	...
      	if (rowCount > 0) {
			...
			table.setURL("${contextPath}/service/simpleAction/PAY0200206S");
			table.setAsync(false);
			table.setRequestMethod("POST");
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.apprStateCD_emp, index[i]) != "20") {
					alert("<fmt:message key="PAY02002A.msg1"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="PAY02002A.msg2"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay02/approvalResultBrand_err");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function Reject() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				//개별승인상태가 승인요청일 경우만 가능
				if(grid.getCellText(_col.apprStateCD_emp, index[i]) != "20") {
					alert("<fmt:message key="PAY02002A.msg3"/>");
					return;
				}
				if(grid.getCellText(_col.apprCommt, index[i]) == "") {
					alert("<fmt:message key="PAY02002A.msg4"/>");
		...
		if(!confirm("<fmt:message key="PAY02002A.msg5"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay02/rejectResultBrand_err");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "PAY0200205S");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("empID", "${params.empID}");
	  	table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("empID", "${params.empID}");
		...
		var headerText = [
				...
				, "<fmt:message key="PAY02002A.rageSphereCD"/>"
				, "<fmt:message key="PAY02002A.officeCD"/>"
				, "<fmt:message key="PAY02002A.empNm"/>"
				, "<fmt:message key="PAY02002A.venueCD"/>"
				, "<fmt:message key="PAY02002A.venueNm"/>"
				, "<fmt:message key="PAY02002A.venueGradCD"/>"
				, "<fmt:message key="PAY02002A.segmentCD"/>"
				, "<fmt:message key="PAY02002A.subSegmentCD"/>"
				, "<fmt:message key="PAY02002A.addrTpCD1"/>"
				, "<fmt:message key="PAY02002A.addrTpCD2"/>"
				, "<fmt:message key="PAY02002A.kindTpCD5"/>"
				, "<fmt:message key="PAY02002A.prdNm"/>"
				, "<fmt:message key="PAY02002A.eu"/>"
				, "<fmt:message key="PAY02002A.factoryPrice"/>"
				, "<fmt:message key="PAY02002A.qtyUnitCD"/>"
				, "<fmt:message key="PAY02002A.planQty"/>"
				, "<fmt:message key="PAY02002A.planUnitAmt"/>"
				, "<fmt:message key="PAY02002A.rfidQty"/>"
				, "<fmt:message key="PAY02002A.resultTpCD"/>"
				, "<fmt:message key="PAY02002A.resultQty"/>"
				, "<fmt:message key="PAY02002A.resultUnitAmt"/>"
				, "<fmt:message key="PAY02002A.guidAmt"/>"
				, "<fmt:message key="PAY02002A.payAmt"/>"
				, "<fmt:message key="PAY02002A.targetQty"/>"
				, "<fmt:message key="PAY02002A.sdxTpCD"/>"
				, "<fmt:message key="PAY02002A.targetRate"/>"
				, "<fmt:message key="PAY02002A.gsvRate"/>"
				, "<fmt:message key="PAY02002A.commt"/>"
				, "<fmt:message key="PAY02002A.apprCommt"/>"
				, "<fmt:message key="PAY02002A.verifyErrCommt"/>"
				, "<fmt:message key="PAY02002A.apprStateCDName_emp"/>"
				, "<fmt:message key="PAY02002A.verifyStateCDName"/>"
		];
		
		table.setParameter("templateFile", "PAY02002A");
