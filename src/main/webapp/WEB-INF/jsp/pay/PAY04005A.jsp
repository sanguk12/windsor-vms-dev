						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="PAY04005A.eventYM" />
												</td>
												<td class="td-input">
											...
											<tr>
				                                <td class="td-cond">
													<fmt:message key="PAY04005A.officeCD" />
												</td>
												<td class="td-input">
													<nis:selectbox id="officeCD_S" name="officeCD_S" defaultText="all" category="OFFICECD" />
				                                </td>
				                            	<td class="td-cond">
													<fmt:message key="PAY04005A.payStateCD" />
												</td>
												<td class="td-input" >
													<select id="payStateCD_S" name="payStateCD_S"></select>
				                                </td>
				                                <td class="td-cond">
													<fmt:message key="PAY04005A.keyChkYN" />
												</td>
												<td class="td-input">
													<nis:selectbox id="keyChkYN_S" name="keyChkYN_S" defaultText="all" category="KEYCHKYN"/>
				                                </td>
				                            </tr>
			                            </tbody>
			                        </table>
...
<style type="text/css">
    ...
    .aw-column-21 {width: 90px; text-align:center;}
...
<script type="text/javascript">
	...
	****************************************/
	...
	function PagingSearch() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0400501S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"keymanName;bankCDName;accountNo;payApprDate;payCommt;"+
	  			"payStateCDName;keyChkYN;prdCD;keymanID;bankCD;"+
	  			"holderNm;payStateCD;keyChkYNCD;rowNum;totalCount;");
		...
		table.setParameter("dkmdTpCD", $("#dkmdTpCD_S").val());
	  	...
	  	table.setParameter("keyChkYN", $("#keyChkYN_S").val());
...
// 	  	if (grid != null) grid.clear();
	...
	function Approval() {
		...
		for (var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				var keyChkYNCD = grid.getCellText(_col.keyChkYNCD, index[i]);
				if(keyChkYNCD != "0" || keyChkYNCD != "3"){
					...
					venueNms[keyChkCount] = grid.getCellText(_col.venueNm, index[i]);
					prdNms[keyChkCount] = grid.getCellText(_col.prdNm, index[i]);
					keyChkCount++;
				}
			}
		}
		
		if(count==0&&keyChkCount==0) {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(payStateCD != "20") {
					alert("<fmt:message key="PAY04005A.msg1"/>");
					return;
				}
			}
		}
		...
		var errPrd = "";
		for(var i=0; i<keyChkCount;i++){
			errPrd += venueNms[i] + "-" + prdNms[i] + "\n";				
		}
		
		if(keyChkCount>0){
			if(count==0){
				alert("<fmt:message key="PAY04005A.msg7"/>");
				return;
			}else{
				if(!confirm(errPrd.replace(/&amp;/g, '&')+"\n<fmt:message key="PAY04005A.msg6"/>")) {
       				return;
       			}
			}
		}else{
	...
	function Reject() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				var payStateCD = grid.getCellText(_col.payStateCD, index[i]);
				if(payStateCD != "20") {
					alert("<fmt:message key="PAY04005A.msg3"/>");
					return;
				}
				if(grid.getCellText(_col.payCommt, index[i]) == "") {
					alert("<fmt:message key="PAY04005A.msg4"/>");
		...
		if(keyChkCount>0){
			if(count==0){
				...
				if(!confirm(errPrd.replace(/&amp;/g, '&')+"\n<fmt:message key="PAY04005A.msg8"/>")) {
       				return;
       			}
			}
		}else{
			if(!confirm("<fmt:message key="PAY04005A.msg5"/>")) {
       			return;
       		}
		}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay04/rejectPayBrand");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="PAY04005A.activityCD"/>"
		, "<fmt:message key="PAY04005A.rageSphereCD"/>"
		, "<fmt:message key="PAY04005A.officeCD"/>"
		, "<fmt:message key="PAY04005A.empNm"/>"
		, "<fmt:message key="PAY04005A.venueCD"/>"
		, "<fmt:message key="PAY04005A.venueNm"/>"
		, "<fmt:message key="PAY04005A.venueGradCD"/>"
		, "<fmt:message key="PAY04005A.segmentCD"/>"
		, "<fmt:message key="PAY04005A.subSegmentCD"/>"
		, "<fmt:message key="PAY04005A.addrTpCD1"/>"
		, "<fmt:message key="PAY04005A.addrTpCD2"/>"
		, "<fmt:message key="PAY04005A.kindTpCD5"/>"
		, "<fmt:message key="PAY04005A.prdNm"/>"
		, "<fmt:message key="PAY04005A.payAmt"/>"
		, "<fmt:message key="PAY04005A.keymanName"/>"
		, "<fmt:message key="PAY04005A.bankCDName"/>"
		, "<fmt:message key="PAY04005A.accountNo"/>"
		, "<fmt:message key="PAY04005A.payApprDate"/>"
		, "<fmt:message key="PAY04005A.payCommt"/>"
		, "<fmt:message key="PAY04005A.payStateCDName"/>"
		, "<fmt:message key="PAY04005A.keyChkYN"/>"
	];
