									...
									<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
						        		...
						        		<tbody>
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="PAY04001B.msg1" />
												</td>
				                            ...
				                            <tr>
				                            	<td></td>
												<td class="td-cond">
													<fmt:message key="PAY04001B.msg2" />
												</td>
												<td class="td-input" style="text-align:right; padding-right:5px;">
...
<style type="text/css">
    ...
    #grid_reg .aw-column-7 {width: 100px; text-align:left;}
    #grid_reg .aw-column-8 {width: 130px; text-align:left;}
...
<script language="javascript">
	...
	function calculatePayAmt_sum() {
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0400102S");
	  	...
	  	table.request();
  		tpayAmt = parseFloat(table.getData(0,0));
		...
		var payAmt_diff = 0;
		payAmt_diff = tpayAmt - payAmt_sum;
		$("#payAmt_diff").text(formatNum(payAmt_diff));
	}
	
	var keyChkSave_btn = new AW.UI.Button;
	keyChkSave_btn.setId("keyChkSave_btn");
	keyChkSave_btn.setControlText("<fmt:message key="button.keyChkSave" />");
	keyChkSave_btn.refresh();
	
	keyChkSave_btn.onControlClicked = function(event) {
...
<script type="text/javascript">
	...
	****************************************/
	...
	function Search() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0100106S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
...
// 	  	if (grid != null) grid.clear();
	...
	function Search_reg() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/RLT0100107S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			"checkbox;keymanID;keymanNm;birthDate;bankCDName;"+
	  			"accountNo;payAmt;keyChkYN;keyChkNote;pkeymanYN;"+
	  			"holderNm;bankCD;keyChkYNCD;");
	  	table.setParameter("format",
	  			"str;str;str;str;str;"+
	  			"str;num;str;str;str;"+
	  			"str;str;str;");
...
// 	  	if (grid_reg != null) grid_reg.clear();
      	...
      	if (rowCount > 0) {
      		...
      		for (var i=0; i<rowCount; i++) {
      			data[i] = [
					...
					, table.getData(_col_reg.keyChkYN,i)
					, table.getData(_col_reg.keyChkNote,i)
					, table.getData(_col_reg.pkeymanYN,i)
					, table.getData(_col_reg.holderNm,i)
					, table.getData(_col_reg.bankCD,i)
					, table.getData(_col_reg.keyChkYNCD, i)
					, table.getData(_col_reg.keyChkYNCD, i)
					, "U"
      			];
      			
      			var keyChkYNCD = table.getData(_col_reg.keyChkYNCD, i);
      			if(keyChkYNCD == "0"){
      				grid_reg.getCellTemplate(_col_reg.keyChkYN, i).setStyle("background", "#ffffff");
      			}else if(keyChkYNCD == "2"){
      				grid_reg.getCellTemplate(_col_reg.keyChkYN, i).setStyle("background", "#ffffff");
      			}else{
      				grid_reg.setPopupTemplate(new AW.Templates.Text, _col_reg.keyChkYN, i);
      				grid_reg.getCellTemplate(_col_reg.keyChkYN, i).setStyle("background", "#ececec");
      			}
	...
	function Add() {
		if(opener.$("#resultWriteYN").val() == "N") {
			alert("<fmt:message key="PAY04001B.msg3" />");
			return;
		}
		...
		var venueCD = "${params.venueCD}";
		//지급상태가 등록, 반려, 보류(10:등록, 40:반려, 50:보류)가 아닌경우에는 이동 불가
		var payStateCD = "${params.payStateCD}";
		if(payStateCD!="" && payStateCD!="10" && payStateCD!="40" && payStateCD!="50") {
			alert("<fmt:message key="PAY04001B.msg4"><fmt:param value='"+venueCD+"' /></fmt:message>");
			return;
		}
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				grid_reg.setCellText("미확인", _col_reg.keyChkYN, maxRow_reg-1);
				grid_reg.setCellText("", _col_reg.keyChkNote, maxRow_reg-1);
				grid_reg.setCellText("0", _col_reg.keyChkYNCD, maxRow_reg-1);
				grid_reg.setCellText("0", _col_reg.keyChkYNChg, maxRow_reg-1);
	...
	function Remove() {
		...
		for(var i=0; i<index_reg.length; i++) {
			if (grid_reg.getCellValue(_col_reg.checkbox, index_reg[i])) {
				var keyChkYNCD = grid_reg.getCellText(_col_reg.keyChkYNCD, index_reg[i]);
				if(keyChkYNCD=="1" || keyChkYNCD=="3"){
					alert("<fmt:message key="PAY04001B.msg8" />");
					return;
				}
				//remove시 키맨정보 다시 조회
				var table = new AW.XML.Table;
	...
	function Save() {
		...
		var payStateCD = "${params.payStateCD}";
		if(payStateCD!="" && payStateCD!="10" && payStateCD!="40" && payStateCD!="50") {
			alert("<fmt:message key="PAY04001B.msg5"><fmt:param value='"+venueCD+"' /></fmt:message>");
			return;
		}
		//업소지급액이 0이 아니면 저장불가
		var payAmt_diff = Number(unformat($("#payAmt_diff").text())); 
		if(payAmt_diff != 0) {
			alert("<fmt:message key="PAY04001B.msg6"/>");
			return;
		}
		for (var i=0; i<maxRow_reg; i++) {
			if(grid_reg.getCellText(_col_reg.keyChkYNChg, i) == "2"){
				if(grid_reg.getCellText(_col_reg.keyChkNote, i) == ""){
					alert("<fmt:message key="PAY04001B.msg7"/>");
					return;
				}
			}
		}
		...
		for (var i=0; i<maxRow_reg; i++) {
			...
			keyChkYNs[count] = grid_reg.getCellText(_col_reg.keyChkYNChg, i);
			keyChkNotes[count] = grid_reg.getCellText(_col_reg.keyChkNote, i);
			count++;
		}
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/rlt01/saveResultKeymanVenue");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", "${params.eventYM}");
	...
	function keyChkNoteCtrl(value, row){
		if(value=="2"){
			grid_reg.setCellEditable(true, _col_reg.keyChkNote,row);
		}else{
			grid_reg.setCellEditable(false, _col_reg.keyChkNote,row);
		}
	}
	...
	*****************************************************/
	var columns = [
		""
		, "<fmt:message key="PAY04001B.keymanID"/>"
		, "<fmt:message key="PAY04001B.keymanNm"/>"
		, "<fmt:message key="PAY04001B.birthDate"/>"
		, "<fmt:message key="PAY04001B.bankCDName"/>"
		, "<fmt:message key="PAY04001B.accountNo"/>"
		, "<fmt:message key="PAY04001B.pkeymanYN"/>"
	];
	...
	var columns_reg = [
		...
		, "<fmt:message key="PAY04001B.bankCDName"/>"
		, "<fmt:message key="PAY04001B.accountNo"/>"
		, "<fmt:message key="PAY04001B.payAmt"/>"
		, "<fmt:message key="PAY04001B.keyChkYN"/>"
		, "<fmt:message key="PAY04001B.keyChkNote"/>"
	];
	var grid_reg = null;
	function createGrid_reg() {
		...
		grid_reg.setCellTemplate(new AW.Templates.Combo, _col_reg.keyChkYN);
		
		grid_reg.setPopupTemplate(function(col, row) {
			if (col == _col_reg.keyChkYN) {
	        	var table = new AW.XML.Table;
...
// 	            }
	            ...
	            list.onItemMouseDown = function(event, i) {
	            	try {
			            ...
			            grid_reg.setCellText(text, col, row);  
			            grid_reg.setCellText(value[i], _col_reg.keyChkYNChg, row);
