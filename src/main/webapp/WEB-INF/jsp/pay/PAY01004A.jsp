						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="PAY01004A.eventYM" />
												</td>
												<td class="td-input">
											...
											<tr>
												<td class="td-cond">
													<fmt:message key="PAY01004A.subSegmentCD" />
												</td>
												<td class="td-input">
				                                ...
				                                <td class="td-cond">
													<fmt:message key="PAY01004A.overGuidAmt" />
												</td>
												<td class="td-input">
				                            ...
				                            <tr>
				                            	<td class="td-cond">
													<fmt:message key="PAY01004A.addrTpCD1" />
												</td>
												<td class="td-input">
				                            ...
				                            <tr>    
												<td class="td-cond">
													<fmt:message key="PAY01004A.verifyStateCD" />
												</td>
                	...
                	<td height="1">
                		<table border="0" cellspacing="0" cellpadding="0" width="100%" >
			        		...
			        		<tbody>
			        			<tr>
									<td class="tit_text01">
										<fmt:message key="PAY01004A.myAppr" />
									</td>
									<td class="tit_text01">
										<span id="myAppr" />
	                                </td>
...
<!-- 						<input type="hidden" id="resultWriteYN" name="resultWriteYN" /> -->
						...
						<input type="hidden" id="apprTpID" name="apprTpID" value="000002" /><!-- 검증라인 -->
					</td>
				</tr>
			</table>
		</form>
...
// 		getResultWriteYN();
		...
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0100401S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"verifyStateCDName;prdCD;resultTpCD;sdxTpCD;verifyStateCD;"+
	  			"keymanID;bankCD;sdxYN;");
...
// 	  	if (grid != null) grid.clear();
	...
	function RequestApp() {
		if($("#myAppr").text() == "") {
			alert("<fmt:message key="PAY01004A.msg1"/>");
			return;
		}
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.verifyStateCD, index[i]) != "10") {
					alert("<fmt:message key="PAY01004A.msg2"/>");
					return;
				}
				if (grid.getCellText(_col.keymanID, index[i]) == "") {
					alert("<fmt:message key="PAY01004A.msg3" />"); 
				...
				if(resultUnitAmt > guidAmt ||
					...
					if(grid.getCellText(_col.commt, index[i]) == "") {
						alert("<fmt:message key="PAY01003A.msg7"/>");
		...
		if(!confirm("<fmt:message key="PAY01004A.msg4"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay01/requestAppResultBrand_err");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("empID", "${params.empID}");
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(grid.getCellText(_col.verifyStateCD, index[i]) != "20") {
					alert("<fmt:message key="PAY01004A.msg5"/>");
					return;
				}
			}
		}
		
		if(!confirm("<fmt:message key="PAY01004A.msg6"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay01/clearRequestAppResultBrand_err");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "PAY0100401S");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
	  	table.setParameter("empID", "${params.empID}");
	  	table.setParameter("activityCD", $("#activityCD_S").val());
	  	table.setParameter("venueGradCD", $("#venueGradCD_S").val());
		...
		var headerText = [
			"<fmt:message key="PAY01004A.activity"/>"
			, "<fmt:message key="PAY01004A.venueCD"/>"
			, "<fmt:message key="PAY01004A.venueNm"/>"
			, "<fmt:message key="PAY01004A.venueGradCD"/>"
			, "<fmt:message key="PAY01004A.segmentCD"/>"
			, "<fmt:message key="PAY01004A.subSegmentCD"/>"
			, "<fmt:message key="PAY01004A.addrTpCD1"/>"
			, "<fmt:message key="PAY01004A.addrTpCD2"/>"
			, "<fmt:message key="PAY01004A.kindTpCD5"/>"
			, "<fmt:message key="PAY01004A.prdNm"/>"
			, "<fmt:message key="PAY01004A.eu"/>"
			, "<fmt:message key="PAY01004A.factoryPrice"/>"
			, "<fmt:message key="PAY01004A.qtyUnitCD"/>"
			, "<fmt:message key="PAY01004A.planQty"/>"
			, "<fmt:message key="PAY01004A.planUnitAmt"/>"
			, "<fmt:message key="PAY01004A.rfidQty"/>"
			, "<fmt:message key="PAY01004A.resultTpCD"/>"
			, "<fmt:message key="PAY01004A.resultQty"/>"
			, "<fmt:message key="PAY01004A.resultUnitAmt"/>"
			, "<fmt:message key="PAY01004A.guidAmt"/>"
			, "<fmt:message key="PAY01004A.payAmt"/>"
			, "<fmt:message key="PAY01004A.targetQty"/>"
			, "<fmt:message key="PAY01004A.sdxTpCD"/>"
			, "<fmt:message key="PAY01004A.targetRate"/>"
			, "<fmt:message key="PAY01004A.gsvRate"/>"
			, "<fmt:message key="PAY01004A.commt"/>"
			, "<fmt:message key="PAY01004A.keymanID"/>"
			, "<fmt:message key="PAY01004A.bankCDName"/>"
			, "<fmt:message key="PAY01004A.accountNo"/>"
			, "<fmt:message key="PAY01004A.verifyErrCommt"/>"
			, "<fmt:message key="PAY01004A.verifyStateCD"/>"
		];
		
		table.setParameter("templateFile", "PAY01004A");
	...
	*****************************************************/
	...
	function createGrid() {
	    ...
	    grid.setCellFormat([
						...
						, str, num5, num, str, num, num, num, str, num, num
						, num, num, num5, str, num, num, str, str, str, str
						, str, html]);
