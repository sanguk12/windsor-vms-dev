						        		...
						        		<tbody>
				                            <tr>
												<td class="td-cond-required">
													<fmt:message key="PAY04006A.eventYM" />
												</td>
												<td class="td-input">
						...
						<table border="0" cellpadding="0" cellspacing="0" width="100%" style="border:#e1dcbe solid 1px; table-layout:fixed;">
			        		...
			        		<tbody>
	                            <tr>
	                            	<td></td>
	                            	<td class="td-cond-report">
										<fmt:message key="PAY04006A.transferDueDate" />
									</td>
									<td class="td-input">
...
<style type="text/css">
    ...
    .aw-column-21 {width: 200px; text-align:left;}
...
<script type="text/javascript">
	...
	****************************************/
	...
	function PagingSearch() {
		var table = new AW.XML.Table;
	  	table.setURL("${contextPath}/service/simpleAction/PAY0400601S");
	  	table.setAsync(false);
	  	table.setRequestMethod("POST");
	  	table.setParameter("outParamKey",
	  			...
	  			"transferDate;payCommt;payStateCDName;prdCD;keymanID;"+
	  			"bankCD;holderNm;payStateCD;rowNum;totalCount;");
...
// 	  	if (grid != null) grid.clear();
	...
	function FinishPay() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(payStateCD != "30") {
					alert("<fmt:message key="PAY04006A.msg1"/>");
					return;
				}
				if(grid.getCellText(_col.transferDate, index[i]) == "") {
					alert("<fmt:message key="PAY04006A.msg3"/>");
					$("#transferDate").focus();
					return;
				}
			}
		...
		if(!confirm("<fmt:message key="PAY04006A.msg2"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay04/finishPayBrand");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				...
				if(payStateCD != "30" && payStateCD != "20" && payStateCD != "10") {
					alert("<fmt:message key="PAY04006A.msg4"/>");
					return;
				}
	...
	function Excel() {
		...
		table.setAsync(false); 
		table.setRequestMethod("POST");
		table.setParameter("queryKey", "PAY0400602S");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("dkmdTpCD", $("#dkmdTpCD_S").val());
	  	table.setParameter("rageSphereCD", $("#rageSphereCD_S").val());
	  	table.setParameter("officeCD", $("#officeCD_S").val());
		...
		var headerText = [
			"<fmt:message key="PAY04006A.eventYM"/>"
			, "<fmt:message key="PAY04006A.incntTpCDName"/>"
			, "<fmt:message key="PAY04006A.activityCD"/>"
			, "<fmt:message key="PAY04006A.programCDName"/>"
			, "<fmt:message key="PAY04006A.payApprDate"/>"
			, "<fmt:message key="PAY04006A.transferDueDate"/>"
			, "<fmt:message key="PAY04006A.transferDate"/>"
			, "<fmt:message key="PAY04006A.payCommt"/>"
			, "<fmt:message key="PAY04006A.dkmdTpCDName"/>"
			, "<fmt:message key="PAY04006A.rageSphereCDName"/>"
			, "<fmt:message key="PAY04006A.officeCDName"/>"
			, "<fmt:message key="PAY04006A.empNm"/>"
			, "<fmt:message key="PAY04006A.venueCD"/>"
			, "<fmt:message key="PAY04006A.venueNm"/>"
			, "<fmt:message key="PAY04006A.prdNm"/>"
			, "<fmt:message key="PAY04006A.keymanID"/>"
			, "<fmt:message key="PAY04006A.keymanNm"/>"
			, "<fmt:message key="PAY04006A.regIDNo"/>"
			, "<fmt:message key="PAY04006A.addr"/>"
			, "<fmt:message key="PAY04006A.phoneNo"/>"
			, "<fmt:message key="PAY04006A.bankCD"/>"
			, "<fmt:message key="PAY04006A.accountNo"/>"
			, "<fmt:message key="PAY04006A.holderNm"/>"
			, "<fmt:message key="PAY04006A.payAmt"/>"
		];
		
		table.setParameter("templateFile", "PAY04006A");
	...
	function SavePayComm() {
		...
		for(var i=0; i<index.length; i++) {
			if (grid.getCellValue(_col.checkbox, index[i])) {
				//지급의견필수 메세지
				if(grid.getCellText(_col.payCommt, index[i]) == "") {
					alert("<fmt:message key="PAY04006A.msg7"/>");
					setCellFocus(grid, _col.payCommt, index[i]);
					return;
				}
			}
		...
		if(!confirm("<fmt:message key="PAY04006A.msg8"/>")) {
       		return;
       	}
		
		var table = new AW.XML.Table;
		table.setURL("${contextPath}/service/pay04/savePayCommBrand");
		table.setAsync(false);
		table.setRequestMethod("POST");
		table.setParameter("eventYM", $("#yearCD_S").val()+$("#monthCD_S").val());
		table.setParameter("venueCDs", venueCDs);
	...
	*****************************************************/
	var columns = [
		...
		, "<fmt:message key="PAY04006A.venueCD"/>"
		, "<fmt:message key="PAY04006A.venueNm"/>"
		, "<fmt:message key="PAY04006A.venueGradCD"/>"
		, "<fmt:message key="PAY04006A.segmentCD"/>"
		, "<fmt:message key="PAY04006A.subSegmentCD"/>"
		, "<fmt:message key="PAY04006A.addrTpCD1"/>"
		, "<fmt:message key="PAY04006A.addrTpCD2"/>"
		, "<fmt:message key="PAY04006A.kindTpCD5"/>"
		, "<fmt:message key="PAY04006A.prdNm"/>"
		, "<fmt:message key="PAY04006A.payAmt"/>"
		, "<fmt:message key="PAY04006A.keymanName"/>"
		, "<fmt:message key="PAY04006A.bankCDName"/>"
		, "<fmt:message key="PAY04006A.accountNo"/>"
		, "<fmt:message key="PAY04006A.payApprDate"/>"
		, "<fmt:message key="PAY04006A.transferDueDate"/>"
		, "<fmt:message key="PAY04006A.transferDate"/>"
		, "<fmt:message key="PAY04006A.payCommt"/>"
		, "<fmt:message key="PAY04006A.payStateCDName"/>"
	];
	...
	function createGrid() {
    	...
    	grid.setCellTemplate(calendarDue, _col.transferDurDate);
